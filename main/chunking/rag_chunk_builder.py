...
from cross_reference_extractor import annotate_chunks_with_cross_refs
...
def annotate_inference_links(chunks: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return annotate_chunks_with_cross_refs(chunks)
...
def build_rag_payload(sections_path: Path, assets_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "counts": {
            ...
            "chunks_with_cross_refs": sum(1 for chunk in chunks if chunk.get("cross_refs")),
            "total_cross_refs": sum(len(chunk.get("cross_refs") or []) for chunk in chunks),
        },
        "chunks": chunks,
    }
