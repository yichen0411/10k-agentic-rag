...
- `0525_redo/chunking/` 包含 PDF 解析、章节划分、资产提取、文本块构建、向量索引、离线 VLM 表格解析、表格摘要向量索引，以及检索命中率评估。
- `0525_redo/inference/` 包含运行时 RAG 推理与 RAGAS 风格答案评估。
- `chunk_studio/` 封装同一流水线，用于上传、可视化、VLM 解析检查与问答。

当前 Apple FY2025 输出包括：

- 菜单引导章节：`0525_redo/chunking/AAPL_FY2025_menu_guided_sections.json`
- 章节关联资产：`0525_redo/chunking/AAPL_FY2025_section_assets.json`
- 纯文本 RAG 块：`0525_redo/chunking/AAPL_FY2025_rag_chunks.json`
...
- 离线 VLM 表格解析：`0525_redo/chunking/vlm_table_parse.py`
- 表格摘要索引构建器：`0525_redo/chunking/build_table_vector_db.py`
- 推理脚本：`0525_redo/inference/text_vector_rag_inference.py`
- 混合推理评估：`0525_redo/inference/run_mixed_inference_eval.py`
- RAGAS 风格混合评估：`0525_redo/inference/eval_msft_mixed_ragas_style.py`
- MSFT 混合测试集：`0525_redo/common/msft_fy2025_mixed_15_inference_test.json`
- MSFT 表格测试集：`0525_redo/common/msft_fy2025_parsed_table_test_questions.json`
- MSFT 表格评估：`0525_redo/inference/eval_msft_table_test_questions.py`
- 推理评估脚本：`0525_redo/inference/eval_single_questions_ragas_style.py`
...
脚本：`0525_redo/chunking/vlm_table_parse.py`
...
脚本：`0525_redo/chunking/build_table_vector_db.py`
...
Chunk Studio 上传后磁盘上 PDF 为 `source.pdf`，但建索引时通过 workspace `metadata.json` 的 `original_filename` 解析 canonical 文件名（`0525_redo/chunking/filing_metadata.py`）。已有 workspace 可用 `0525_redo/chunking/patch_workspace_source_files.py` 批量修补。
...
测试集：`0525_redo/common/msft_fy2025_parsed_table_test_questions.json`
...
- `0525_redo/inference/msft_fy2025_table_test_eval.json` — 旧流水线
- `0525_redo/inference/msft_fy2025_table_test_eval_v2.json` — 双路径流水线
- 对应 `.jsonl` 日志，含每题流水线路径与延迟

### MSFT FY2025 混合推理（15 题：10 表格 + 5 文本）

测试集：`0525_redo/common/msft_fy2025_mixed_15_inference_test.json`
