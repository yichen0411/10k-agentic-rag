...
class CrossReferenceExtractorTests(unittest.TestCase):
    def test_refer_to_note_with_title(self):
        text = "Refer to Note 10 – Debt of the Notes to Financial Statements (Part II, Item 8 of this Form 10-K)."
        refs = extract_cross_references(text)
        self.assertEqual(len(refs), 1)
        self.assertEqual(refs[0]["cue_type"], "refer_to")
        self.assertEqual(refs[0]["target_type"], "note_with_location")
        self.assertIn("Note 10", refs[0]["target_label"])

    def test_see_item_with_heading(self):
        text = 'see Item 1A of this Form 10-K under the heading "Risk Factors."'
        refs = extract_cross_references(text)
        cue_types = {ref["cue_type"] for ref in refs}
        self.assertIn("see", cue_types)
        self.assertIn("under_the_heading", cue_types)

    def test_discussed_in_note(self):
        text = "As discussed in Note 7 to the financial statements, the Company is subject to income taxes."
        refs = extract_cross_references(text)
        self.assertEqual(len(refs), 1)
        self.assertEqual(refs[0]["target_type"], "note_simple")
        self.assertEqual(refs[0]["target_label"], "Note 7")

    def test_incorporated_by_reference_proxy_before_cue(self):
        text = "will be included in the 2026 Proxy Statement, and is incorporated herein by reference."
        refs = extract_cross_references(text)
        self.assertTrue(any(ref["cue_type"] == "incorporated_by_reference" for ref in refs))
        self.assertTrue(any(ref["target_type"] == "proxy_statement" for ref in refs))

    def test_ignores_non_filing_refer_to(self):
        text = "quarters, months and periods refer to the Company's fiscal years ended in September."
        refs = extract_cross_references(text)
